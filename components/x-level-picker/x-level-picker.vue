<template>
	<view :class="['lb-picker', inline ? 'lb-picker-inline' : '']">
		...
		<view v-if="visible && showMask && !inline"
			...
			:style="{ backgroundColor: maskBgColor, zIndex: zIndex - 1 }"
			@tap.stop="handleMaskTap" @touchmove.stop.prevent="moveHandle">
		</view>

		<view v-if="visible || inline" ref="container" 
			:class="[ 'lb-picker-container', !inline ? 'lb-picker-container-fixed' : '', animation ? 'lb-picker-container-animation' : '', containerVisible ? 'lb-picker-container-show' : '' ]"
			:style="{ borderRadius: `${radius} ${radius} 0 0`, zIndex: zIndex }">
			<view v-if="showHeader" class="lb-picker-header">
				...
				<view class="lb-picker-header-actions">
					<!-- 取消 -->
					<view class="lb-picker-action lb-picker-action-cancel" @tap.stop="handleCancel">
						<slot v-if="$slots['cancel-text']" name="cancel-text">
						</slot>
						<text v-else class="lb-picker-action-cancel-text"
					...
					<!-- 确定 -->
					<view class="lb-picker-action lb-picker-action-confirm" @tap.stop="handleConfirm">
						<slot v-if="$slots['confirm-text']" name="confirm-text"> </slot>
						<text v-else class="lb-picker-action-confirm-text"
			...
			<view :class="[ 'lb-picker-content', safeAreaInsetBottom ? 'lb-picker-content-safe-buttom' : '' ]">
				...
				<view class="lb-picker-content-main" :style="{ height: pickerContentHeight }">
					...
					<selector-picker v-if="mode === 'selector' && !loading && !isEmpty" :ref="mode" :value="value"
						...
						:column-style="columnStyle" :active-column-style="activeColumnStyle" :align="align"
						:press-enable="pressEnable" :press-time="pressTime" :formatter="formatter"
						@change="handleChange">
					</selector-picker>
					...
					<multi-selector-picker v-if="mode === 'multiSelector' && !loading && !isEmpty" :ref="mode"
						...
						:height="pickerContentHeight" :inline="inline" :column-style="columnStyle"
						:active-column-style="activeColumnStyle" :align="align" :press-enable="pressEnable"
						:press-time="pressTime" :formatter="formatter" @change="handleChange">
					...
					<!-- 日期选择 -->
					<date-selector-picker v-if="mode === 'dateSelector' && !loading" :ref="mode" :value="value"
						:mode="mode" :visible="visible" :height="pickerContentHeight" :inline="inline"
						:column-style="columnStyle" :active-column-style="activeColumnStyle" :align="align"
						:formatter="formatter" :format="format" :display-format="displayFormat" :start-date="startDate"
						:end-date="endDate" :default-time-limit="defaultTimeLimit" :is-show-chinese="isShowChinese"
						:ch-config="pickerChConfig" :filter="filter" @change="handleChange">
...
<script>
	...
	const defaultProps = {label: 'label',value: 'value',children: 'children'}
	const defaultChConfig = {year: '年',month: '月',day: '日',hour: '时',minute: '分',second: '秒'}
	// #ifdef APP-NVUE
	...
	export default {
		...
		watch: {
			...
			visible(newVisible) {
				newVisible ? this.$emit('show') : this.$emit('hide')
			},
			props(newProps) {
