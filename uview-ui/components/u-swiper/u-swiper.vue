			...
			<swiper-item class="u-swiper-item" v-for="(item, index) in list" :key="index">
				<view class="u-list-image-wrap" @tap.stop.prevent="listClick(index)" :class="[uCurrent != index ? 'u-list-scale' : '']"
				 :style="{ 
					...
					'padding-left': spaceBetween ? spaceBetween + 'rpx' : '0',
					'padding-right': spaceBetween ? spaceBetween + 'rpx' : '0',
					 }">
					<image class="u-swiper-image u-skeleton-rect" :src="item.pic || item[name] || item " :mode="imgMode" :style="{borderRadius: `${borderRadius}rpx`}"></image>
